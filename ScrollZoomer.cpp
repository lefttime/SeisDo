...
void ScrollZoomer::setCornerWidget( QWidget* cw )
{
  if( cw != d_cornerWidget )
  {
    if( canvas() ) {
      delete d_cornerWidget;
      d_cornerWidget = cw;
...
bool ScrollZoomer::needScrollBar( Qt::Orientation orientation ) const
{
  ...
  bool needed = false;
  switch( mode ) {
  case Qt::ScrollBarAlwaysOn: needed = true; break;

  case Qt::ScrollBarAlwaysOff: needed = false; break;
...
void ScrollZoomer::layoutScrollBars( const QRect &rect )
{
  ...
  if( hScrollBar && hScrollBar->isVisible() &&
    ...
    if( d_cornerWidget ) {
      QRect cornerRect( vScrollBar->pos().x(),
                        hScrollBar->pos().y(),
                        vdim, hdim );
...
void ScrollZoomer::scrollBarMoved( Qt::Orientation orientation, double min, double max )
{
  Q_UNUSED( max );
